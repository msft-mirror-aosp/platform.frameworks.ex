...
public class AdvancedExtenderCreator {
    private static AdvancedExtenderCreator instance;
     ...
     * @since 1.6
     */
    public static synchronized AdvancedExtenderCreator getInstance() {
        if (instance == null) {
            instance = new AdvancedExtenderCreator();
        }
        return instance;
    }
     ...
     * @since 1.6
     */
    public AdvancedExtenderImpl createAdvancedExtender(int extensionMode) {
        return switch (extensionMode) {
            case CameraExtensionCharacteristics.EXTENSION_AUTOMATIC ->
                    new AutoAdvancedExtenderImpl();
            case CameraExtensionCharacteristics.EXTENSION_FACE_RETOUCH ->
                    new BeautyAdvancedExtenderImpl();
            case CameraExtensionCharacteristics.EXTENSION_BOKEH -> new BokehAdvancedExtenderImpl();
            case CameraExtensionCharacteristics.EXTENSION_HDR -> new HdrAdvancedExtenderImpl();
            case CameraExtensionCharacteristics.EXTENSION_NIGHT -> new NightAdvancedExtenderImpl();
            default -> null;
        };
    }
}